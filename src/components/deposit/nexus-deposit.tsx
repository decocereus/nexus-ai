...
const NexusDeposit = ({
  ...
  useEffect(() => {
    onStateChange?.({ status: widget.status, step: widget.step });
  }, [onStateChange, widget.status, widget.step]);

  useEffect(() => {
    if (!widget.activeIntent) return;
    onPreview?.(widget.activeIntent.intent);
  }, [onPreview, widget.activeIntent]);
