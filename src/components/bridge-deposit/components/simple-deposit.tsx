...
const SimpleDeposit = ({
  ...
  useEffect(() => {
    if (status === 'success') {
      onComplete?.(explorerUrls.executeUrl ?? explorerUrls.intentUrl ?? undefined)
    }
  }, [explorerUrls.executeUrl, explorerUrls.intentUrl, onComplete, status])
  ...
  return (
    <div className="flex flex-col items-center w-full gap-y-3 rounded-lg relative">
      {showHistory ? <ViewHistory className="absolute -top-9 right-0" /> : null}
      {/* Sources */}
      <SourceSelect
        token={token ?? 'USDC'}
